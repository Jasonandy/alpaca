...
* @Description：   <p> 据访问层基础支撑类.</p>
* @Author： - Jason   
* @CreatTime：2018年5月10日 下午10:00:51   
* @Modify By：   
...
public abstract class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
    ...
    public PageBean<T> listPage(PageParam pageParam, Map<String, Object> paramMap) {
        ...
        if (isCount != null && "1".equals(isCount.toString())) {
            Map<String, Object> countResultMap = sessionTemplate.selectOne(getStatement(SQL_COUNT_BY_PAGE_PARAM), paramMap);//countResultMap 统计结果数据
            return new PageBean<T>(pageParam.getPageNum(), pageParam.getNumPerPage(), totalCount.intValue(), list, countResultMap);
        } else {
            // 构造分页对象
            return new PageBean<T>(pageParam.getPageNum(), pageParam.getNumPerPage(), totalCount.intValue(), list);
        }
    }
