 ...
 * </html>
 */
package cn.ucaner.alpaca.framework.component.excel.three;
...
import cn.ucaner.alpaca.framework.utils.date.DateFormatUtils;
import cn.ucaner.alpaca.framework.utils.date.DateUtilOne;
		...
		if ("java.lang.String".equals(type) || "String".equals(type)) {
			...
			if (value.length() > 0){//当长度为19(yyyy-MM-dd HH24:mm:ss)或者为14(yyyyMMddHH24mmss)时Date格式转换为yyyyMMddHH24mmss
				if(value.length() == 19 || value.length() == 14){    
					oo[0] = DateUtilOne.string2Date(value, "yyyyMMddHH24mmss");
				}
				else{     //其余全部转换为yyyyMMdd格式
					oo[0] = DateUtilOne.string2Date(value, "yyyyMMdd");
				}
			}
