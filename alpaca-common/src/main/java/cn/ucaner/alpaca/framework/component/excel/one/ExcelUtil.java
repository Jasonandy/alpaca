...
* @ClassName：ExcelUtil   
* @Description：   <p> The <code>ExcelUtil</code> 与 {@link ExcelCell}搭配使用</p>
* @Author： - Jason 
* @CreatTime：2017年10月24日 下午2:34:54   
* @Modify By：   
* @ModifyTime：  
* @Modify marker：   
* @version    V1.0
...
public class ExcelUtil {
	...
	private static HSSFWorkbook workbook3;
    ...
    public static void exportExcel(String[][] datalist, OutputStream out) {
        try {
            ...
            // 生成一个表格
            HSSFSheet sheet = workbook2.createSheet();
    ...
    public static <T> void exportExcel(List<ExcelSheet<T>> sheets, OutputStream out, String pattern) {
        ...
        for (ExcelSheet<T> sheet : sheets) {
            // 生成一个表格
            HSSFSheet hssfSheet = workbook3.createSheet(sheet.getSheetName());
